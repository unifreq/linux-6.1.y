...
/*
 * Refer to RTL8126 datasheet 5.Customizable LED Configuration
...
rtl8126_led_configuration(struct rtl8126_private *tp)
{
	...
	// To be compatible with the old device tree
        ...
        if (ret == 0)
                RTL_W16(tp, LEDSEL0, led_data & LEDSEL_MASK_8126);
	...
	// The new device tree is written as follows:
        ...
        ret = of_property_read_u32(tp->pci_dev->dev.of_node,
                                  "r8126,led1", &led_data);
        if (ret == 0)
                RTL_W16(tp, LEDSEL1, led_data & LEDSEL_MASK_8126);

        ret = of_property_read_u32(tp->pci_dev->dev.of_node,
                                  "r8126,led2", &led_data);
        if (ret == 0)
                RTL_W16(tp, LEDSEL2, led_data & LEDSEL_MASK_8126);

        ret = of_property_read_u32(tp->pci_dev->dev.of_node,
                                  "r8126,led3", &led_data);
        if (ret == 0)
                RTL_W16(tp, LEDSEL3, led_data & LEDSEL_MASK_8126);

        ret = of_property_read_u32(tp->pci_dev->dev.of_node,
                                  "r8126,led-feature", &led_data);
        if (ret == 0)
                RTL_W8(tp, LEDFEATURE, led_data & LEDFEATURE_MASK_8126);
}
